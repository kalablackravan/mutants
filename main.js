...
preload().then(() => {
  ...
  const safe = (n, f) => { try { f(); } catch (e) { console.error("[boot] " + n + " failed:", e); } };
  safe("initArts", initArts); safe("researchLab", researchLab); safe("flaskLab", flaskLab); safe("wall", wall);
  safe("index", () => $$("[data-pt]").forEach(index)); safe("nav", nav); safe("devilCam", devilCam); safe("form", form);
  safe("hotlab", () => { if ($("#hotlabSpots")) hotlab(); });
  requestAnimationFrame(() => requestAnimationFrame(reveal));
  setTimeout(reveal, 60);
}).catch(e => { console.error(e); reveal(); });
